...
// GetForToken method retrieves the details of the user associated with a
// particular activation token. If there is no matching token found, or it has
// expired, this returns a `ErrRecordNotFound` error instead.
func (m UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	// Create a slice containing the query arguments. Notice how we use the [:]
	// operator to get a slice containing the token hash, rather than passing in
	// the array (which is not supported by the pq driver), and that we pass the
...
// Get retrieves the user details based on their ID.
