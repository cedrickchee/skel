...
type config struct {
	port int
	env  string
	// Hold the configuration settings for our database connection pool. For now
	// this only holds the DSN, which we will read in from a command-line flag.
	db struct {
		dsn string
	}
}
...
func main() {
	...
	flag.StringVar(&cfg.db.dsn, "db-dsn", "postgres://skel:pa55word@localhost/skel", "PostgreSQL DSN")
